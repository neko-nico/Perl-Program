...
#创建一个随机的.xsd别用
my $doc = Documents->New("Atoms.xsd");

for (my $i = 0; $i < @points_List; $i += 3) {
	print "point:", $i, ', ', $i+1, ', ', $i+2, ', ', "\n";
	$doc->CreateAtom("C", Point(X => $points_List[$i], Y => $points_List[$i + 1], Z => $points_List[$i+2]));
}

my $point = Point(X => 0.2, Y => 0.2, Z => 0.2);
my $point2 = Point(X => -0.2, Y => 0.2, Z => 0.2);
